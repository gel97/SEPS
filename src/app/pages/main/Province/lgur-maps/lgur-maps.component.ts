...
@Component({
  selector: 'app-lgur-maps',
  templateUrl: './lgur-maps.component.html',
  styleUrls: ['./lgur-maps.component.css'],
})
export class LgurMapsComponent implements OnInit {
  cdra: any[] = [];
  filteredCdra: any[] = [];
  ...
  GetLguMaps(): void {
    this.service.GetLguMaps(this.auth.munCityId, this.auth.setYear).subscribe({
      next: (response) => {
        this.cdra = response;
  ...
  uploadLGUMap(): void {
    ...
    formData.append('MunCityId', this.auth.munCityId);
    ...
    this.service.uploadLguMaps(formData).subscribe({
      next: () => {
        Swal.close();
