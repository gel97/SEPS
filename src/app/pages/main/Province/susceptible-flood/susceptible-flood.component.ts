...
import { SusceptibleFloodService } from 'src/app/shared/Province/SusceptibleFlood.service';
...
@Component({
  selector: 'app-susceptible-flood',
  templateUrl: './susceptible-flood.component.html',
  styleUrls: ['./susceptible-flood.component.css'],
})
export class SusceptibleFloodComponent implements OnInit {
  @ViewChild('closebutton')
  closebutton!: { nativeElement: { click: () => void } };
  constructor(
    private auth: AuthService,
    private service: SusceptibleFloodService,
  ...
  Getsusflood() {
    this.service.GetSusFlood(this.auth.setYear).subscribe({
      next: (response) => {
        this.listSusFlood = <any>response;
      },
      error: (error) => {},
      complete: () => {
        this.GetListMunicipality();
  ...
  FilterList() {
    ...
    this.listMunCity.forEach((a: any) => {
      this.listSusFlood.forEach((b: any) => {
  ...
  AddData() {
    if (isEmptyObject(this.data)) {
      ...
      this.service.AddSusFlood(this.data).subscribe({
        next: (request) => {
  ...
  EditData() {
    this.data.setYear = this.auth.activeSetYear;
    this.service.EditSusFlood(this.data).subscribe({
      next: (request) => {
        this.closebutton.nativeElement.click();
        this.data = {};
      },
