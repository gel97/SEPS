...
import { TravelAndOperationsService } from 'src/app/shared/Province/TravelAndOperations.Service';
...
@Component({
  selector: 'app-travel-ops',
  templateUrl: './travel-ops.component.html',
  styleUrls: ['./travel-ops.component.css'],
})
export class TravelOpsComponent implements OnInit {
  ...
  constructor(
    ...
    private Service: TravelAndOperationsService,
  ...
  GetTrav() {
    {
      this.Service.GetTrav(this.auth.setYear).subscribe({
        next: (response) => {
          this.listTravOps = <any>response;
        },
        error: (error) => {},
        complete: () => {
          this.GetListMunicipality();
  ...
  FilterList() {
    ...
    if (!Array.isArray(this.listMunCity) || !Array.isArray(this.listTravOps)) {
      console.warn('Either listMunCity or listPYAP is not an array yet.');
      return;
    }

    this.listMunCity.forEach((a: any) => {
      this.listTravOps.forEach((b: any) => {
  ...
  get totals() {
    return this.listData.reduce(
      (
        acc: {
          noOfTravAndOps: number;
        },
        item: {
          noOfTravAndOps: any;
        }
      ) => {
        acc.noOfTravAndOps += item.noOfTravAndOps || 0;
  ...
  AddTrav() {
    if (isEmptyObject(this.data)) {
      ...
      this.Service.AddTrav(this.data).subscribe({
        next: (response) => {
  ...
  EditTrav() {
    this.data.setYear = this.auth.activeSetYear;
    this.Service.EditTrav(this.data).subscribe({
      next: (request) => {
        this.data = {};
      },
      complete: () => {
        this.GetTrav(); // ⬅️ Add this
        this.closeMunicipalityBtn.nativeElement.click();
        Swal.fire({
          position: 'center',
          icon: 'success',
  ...
  DeleteTrav(transId: any, index: any, data: any) {
    Swal.fire({
      title: 'Are you sure?',
      ...
      if (result.isConfirmed) {
        this.Service.DeleteTrav(transId).subscribe({
          next: (_data) => {},
          error: (err) => {
