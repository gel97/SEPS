...
@Component({
  ...
  styleUrls: ['./express-mail.component.css'],
})
export class ExpressMailComponent implements OnInit {
  ...
  constructor(
    ...
    private service: ExpressMailService,
  ...
  message = 'Express Mail/ Mailing Services';
  markerObj: any = {};
  sources: any = [];
  newSource: any = {};
  ...
  getSources(): void {
    ...
    const sourceFor = 'expressMail'; // 👈 assign your module name
  ...
  GeneratePDF() {
    ...
    this.reportService.GetComExpressReport(this.pdfComponent.data).subscribe({
      next: (response: any = {}) => {
        reports = response;
  ...
  GetListData() {
    this.service.GetListExpressMail(this.setYear, this.munCityId).subscribe({
      next: (response) => {
  ...
  DeleteData(transId: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.service.DeleteExpressMail(transId).subscribe((request) => {
          this.GetListData();
        });
