...
export class WaterPumpComponent implements OnInit {
  ...
  constructor(private service:ServicesStationService, private auth: AuthService) { }
  ...
  GetList_Station()
  {
    this.service.List_Station(this.menuId, this.setYear, this.munCityId).subscribe({
      next: (response) =>
      {
  ...
  Add_Station()
  {
    this.toValidate.company = this.station.company=="" || this.station.company ==undefined?true:false;
    this.toValidate.stationName = this.station.stationName=="" || this.station.stationName ==undefined?true:false;
    this.toValidate.brgyId = this.station.brgyId=="" || this.station.brgyId ==null?true:false;

    this.station.menuId    = this.menuId;
    this.station.setYear   = this.setYear;
    this.station.munCityId = this.munCityId;

    if(!this.toValidate.company && !this.toValidate.brgyId  && !this.toValidate.stationName)
    {
