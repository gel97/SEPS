...
export class SummaryCommercialComponent implements OnInit {
  searchText: string = '';
  constructor(
    private service: SummCommercialService,
  ...
  Add_Com_Estab() {
    this.toValidate.category =
      this.summ.category == '' || this.summ.category == null ? true : false;
    this.toValidate.type =
      this.summ.type == '' || this.summ.type == undefined ? true : false;
    this.toValidate.count =
      this.summ.count == '' || this.summ.count == undefined ? true : false;
    ...
    if (
      ...
      this.summ.tag = 1;
      this.service.Add_Summ_Estab(this.summ).subscribe(
        (request) => {
          if (!this.isCheck) {
  ...
  UpdateSummary_Estab() {
    this.toValidate.category =
      this.editmodal.category == '' || this.editmodal.category == null
        ...
        : false;
    this.toValidate.type =
      this.editmodal.type == '' || this.editmodal.type == undefined
        ? true
        : false;
    this.toValidate.count =
      this.editmodal.count == '' || this.editmodal.count == undefined
        ? true
        : false;
    // this.toValidate.remarks = this.summ.remarks =="" || this.summ.remarks == null?true:false;
    ...
    if (
      ...
      this.service.Update_Summ_Estab(this.editmodal).subscribe({
        next: (_data) => {
          this.GetList_Summary_CommercialEstab();
          this.editmodal = {};
        },
  ...
  get filteredItems() {
    return this.list_of_Business.filter(
      (item) => item.categoryId == this.summ.category
    );
  }
