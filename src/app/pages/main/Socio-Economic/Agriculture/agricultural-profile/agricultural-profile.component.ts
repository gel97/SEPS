...
export class AgriculturalProfileComponent implements OnInit {
  ...
  GetListAgricultureProfile() {
    this.Service.GetListAgricultureProfile(this.Auth.activeSetYear, this.Auth.munCityId).subscribe({
      next: (response) => {
...
AddAgricultureProfile() {
  this.toValidate.riceIrrigTotal = this.addData.riceIrrigTotal == "" || this.addData.riceIrrigTotal == undefined ? true : false;
  this.toValidate.riceRainTotal = this.addData.riceRainTotal == "" || this.addData.riceRainTotal == undefined ? true : false;

  if (this.toValidate.riceIrrigTotal  == true || this.toValidate.riceRainTotal== true) {
    Swal.fire(
      'Missing Data!',
  ...
  this.Service.AddAgricultureProfile(this.addData).subscribe(_data=>{
    Swal.fire(
      'Good job!',
  ...
  DeleteAgricultureProfile(transId:any) {
    {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.Service.DeleteAgricultureProfile(transId).subscribe(request => {
            this.GetListAgricultureProfile();
            this.addData = {};
          })
          Swal.fire(
