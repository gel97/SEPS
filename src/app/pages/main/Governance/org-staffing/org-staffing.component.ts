...
export class OrgStaffingComponent implements OnInit {
  ...
  GetOrgImage(){
    this.imageService.GetOrg(this.auth.munCityId).pipe(concatMap(item => of(item).pipe(delay(2000))), catchError((error: any, caught: Observable<any>): Observable<any> => {
  ...
  this.fullImageAspectRatio = image.original.size.width / image.original.size.height;
  ...
  ProceedUpload() {
    ...
    this.imageService.UploadOrg(formdata).pipe(
      map(events => {
        switch (events.type) {
